...
class SpmdRlvrPPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if extra_args and "reward_funcs" in extra_args:
            self.reward_funcs = extra_args["reward_funcs"]
        else:
            logger.warning(
                "No reward functions provided in `extra_args`, "
                "using the default math/code reward function of AReaL."
            )
            # TODO: add reward function
            # signature: [prompt: Dict[str, Tensor], data: Dict[str, Tensor]] -> torch.Tensor
    ...
    def _get_rollout_batch(self) -> Dict[str, torch.Tensor]:
        if self.config.use_async:
            ...
            # TODO: merge reward function here
    ...
    def _train_step(self):
        ...
        prompt, rollout = self._get_rollout_batch()
        ...
        # TODO: should revisite this function since logps length is changed
        ...
        def ppo_loss_fn(logits: torch.Tensor, input_data: Dict) -> torch.Tensor:
